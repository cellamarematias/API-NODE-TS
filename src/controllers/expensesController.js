...
export const getExpenses = async (req, res) => {
	try {
		const [rows] = await pool.query('SELECT * FROM gastos ORDER BY idgastos DESC LIMIT 50');
		res.send(rows);
	} catch (error) {
...
export const updateExpense = async (req, res) => {
	...
	if (descripcion === 'Balance a $0') {
		try {
			const [result] = await pool.query('UPDATE gastos SET descripcion = IFNULL(?, descripcion), monto = IFNULL(?, monto), date = IFNULL(?, date), id_usuario = IFNULL(?, id_usuario), , pagado = IFNULL(?, pagado) WHERE idgastos = ?', [descripcion, monto, date, id_usuario, true, id]);
